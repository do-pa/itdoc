...
:::tip 만일 `PathVariable`을 포함하는 경로라면, 예시와 같이 정의하고 [req()](#req)에서
`pathParam()`으로 값을 설정하세요.
...
:::info `itdoc`는 [supertest]를 사용하여 HTTP 요청을 시뮬레이션합니다. 따라서 `app` 매개변수는
[supertest]와 호환되는 Express 인스턴스여야 합니다.
...
:::warning `testFn`에서는 반드시 [describeAPI](#describeapi)에서 전달받은 `apiDoc` 객체를 반환하거나
실행해야 합니다. :::

**예시:**

```javascript
itDoc("모든 사용자를 성공적으로 검색합니다", async () => {
    await apiDoc
        .test()
        .req()
...
:::info 무조건 `test()` -> `req()` -> `res()` 순서로 체이닝해야 합니다.
...
:::tip `field()`를 생략하고 값을 직접 사용할 수 있습니다.

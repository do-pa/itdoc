...
**프로젝트에 Itdoc을 적용하려는데 문서화할 API가 너무 많아 수작업으로 테스트를 명세하기 힘든가요?**
...
<Tabs>
    <TabItem value="auth" label="🔐 인증 API" default>
        ...
        - Expected Response: 상태 코드 200, 비밀 메시지를 담은 JSON 응답과 특정 헤더 포함
        - Test Case: 인증되지 않은 접근
        - Request: 잘못된 인증 헤더와 함께 GET 요청을 보낸다.
        - Expected Response: 상태 코드 401를 반환한다.
        ```
    ...
    <TabItem value="file" label="📁 파일 업로드 API">
        ...
        - Expected: 상태 코드 200, 이미지 URL이 담긴 JSON 응답
        - Test Case: 잘못된 콘텐츠 타입
        - Request: "multipart/form-data" 콘텐츠 타입 없이 POST 요청을 보낸다.
        - Expected Response: 상태 코드 400, 에러 메시지를 담은 JSON 응답
        ```

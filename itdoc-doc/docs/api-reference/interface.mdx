...
:::tip If the path contains a `PathVariable`, define it as shown below and set values with
`pathParam()` in [req()](#req).
...
:::info `itdoc` uses [supertest] to simulate HTTP requests. Thus, the `app` parameter must be an
Express instance compatible with [supertest].
...
:::warning `testFn` must return or execute the `apiDoc` object received from
[describeAPI](#describeapi). :::

**Example:**

```javascript
itDoc("Successfully retrieves all users", async () => {
    await apiDoc
        .test()
        .req()
...
:::info Chain methods in the following mandatory order: `test()` -> `req()` -> `res()`.
...
:::tip You can omit `field()` and directly use values.

However, omitting `field()` means the field description won't be documented, so using `field()` is
recommended.

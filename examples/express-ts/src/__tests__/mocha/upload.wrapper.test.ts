/**
 * Upload API Tests using wrapTest wrapper approach (Mocha version)
 *
...
describe("Upload API - Wrapper Approach (Mocha)", () => {
    ...
    apiTest.withMeta({
        ...
        expect((response.body as any).file).to.have.property("originalname", "test-file.txt")
        expect((response.body as any).file).to.have.property("mimetype", "text/plain")
    })
    ...
    apiTest.withMeta({
        ...
        expect(response.status).to.equal(201)
        expect(response.body).to.have.property("message", "Files uploaded successfully")
        expect((response.body as any).files).to.have.lengthOf(2)
        expect((response.body as any).files[0]).to.have.property("originalname", "test-file.txt")
        expect((response.body as any).files[1]).to.have.property("originalname", "test-image.png")
    })
    ...
    apiTest.withMeta({
        ...
        expect(response.status).to.equal(201)
        expect(response.body).to.have.property("message", "Document uploaded successfully")
        expect((response.body as any).document).to.have.property("title", "Important Document")
        expect((response.body as any).document).to.have.property(
            "description",
            "This is a very important document",
        )
        expect((response.body as any).document.file).to.have.property(
            "originalname",
            "test-file.txt",
        )
    })
    ...
    apiTest.withMeta({
        ...
        expect(response.status).to.equal(201)
        expect(response.body).to.have.property("message", "File uploaded successfully")
        expect((response.body as any).file).to.have.property("originalname", "test-image.png")
        expect((response.body as any).file).to.have.property("mimetype", "image/png")
    })
})
